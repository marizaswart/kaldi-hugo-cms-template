    ...
    return <div>
        ...
        <div className="mw7 center ph3">
          <div className="db mb3">
            <div className="mw7 relative bg-fix-primary mb3">
              <h1 className="f2 f1-l b di lh-title mt0 mb3 white mw6 bg-primary">
      ...
      <div className="bg-off-white pv4">
        <div className="ph3 mw7 center">
          <h2 className="f2 b lh-title mt0 mb2">{entry.getIn(["data", "intro", "heading"])}</h2>
          <p className="mt0 mb4 mw6">{entry.getIn(["data", "intro", "description"])}</p>
          ...
          <div className="flex-ns flex-wrap mhn2-ns mb3">
            {(entry.getIn(["data", "intro", "blurbs"]) || []).map((blurb, index) => <div className="ph2-ns w-50-ns mb4" key={index}>
              ...
              <p className="mt0">{blurb.get("text")}</p>
            </div>)}
          </div>
        </div>
      </div>
      ...
      <div className="mw7 center">
        <div className="mw6 ph3 mb3">
          <h3 className="f3 b lh-title mt0 mb2">{entry.getIn(["data", "main", "heading"])}</h3>
          <p className="mt0">{entry.getIn(["data", "main", "description"])}</p>
        </div>
      </div>
      ...
      <div className="pb4">
        {(entry.getIn(['data', 'testimonials']) || []).map((testimonial, index) => <div className="center mb3 ph3" key={index}>
        	<blockquote className="bg-grey-1 primary pa3 mb3 br1 b mw6 center">
        		<p className="f4 mv0">“{testimonial.get('quote')}”</p>
      ...
      <div className="bg-off-white pv4 ph3">
      	<div className="mw7 center">
      		...
      		<h2 className="f2 b lh-title mt0 mb3">{entry.getIn(['data', 'pricing', 'heading'])}</h2>
      		<p className="mt0 mw6">{entry.getIn(['data', 'pricing', 'description'])}</p>

      		<div className="flex-ns mhn2-ns mw7">
            {(entry.getIn(['data', 'pricing', 'plans']) || []).map((plan, index) => <div className="w-33-ns ph2" key={index}>
              <div className="ph2">

              	<h3 className="b f5 grey-3 tc lh-title mt0 mb3">{plan.get('plan')}</h3>

              	<p className="primary f1 b tc lh-title mt0 center">
              		<span className="f4">$</span>{plan.get('price')}
              	</p>

-              	<p className="b mt0">{plan.get('description')}</p>

              	<ul>
                  {(plan.get('items') || []).map((item, index) => <li key={index}>
                    <p className={index + 1 !== plan.get('items').size ? "pb2 mt0 mb2 divider-grey" : null}>{item}</p>
                  </li>)}
              	</ul>
